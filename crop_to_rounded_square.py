...
OUTPUT_DIR_NAME = "rounded_square_icons"
...
def crop_to_rounded_square():
    ...
    for filename in os.listdir(current_dir):
        ...
        try:
            with Image.open(file_path) as img:
                ...
                # 3. 创建圆角蒙版 (4倍超采样渲染，抗锯齿)
                scale_up = 4
                mask_size = SQUARE_SIZE * scale_up
                mask = Image.new("L", (mask_size, mask_size), 0)
                draw = ImageDraw.Draw(mask)
                draw.rounded_rectangle((0, 0, mask_size, mask_size),
                                       radius=CORNER_RADIUS * scale_up, fill=255)
                # 缩放回原始尺寸（平滑无锯齿的圆角边缘）
                mask = mask.resize((SQUARE_SIZE, SQUARE_SIZE), Image.Resampling.LANCZOS)

                # 4. 应用圆角蒙版
                rounded_img = Image.new("RGBA", (SQUARE_SIZE, SQUARE_SIZE), (0, 0, 0, 0))
                rounded_img.paste(img_cropped, (0, 0), mask=mask)

                # 5. 保存为 PNG 格式到新文件夹
                base_name = os.path.splitext(filename)[0]
                save_path = os.path.join(output_dir, f"{base_name}.png")
                rounded_img.save(save_path, "PNG")
    ...
    print(f"\n全部完成！共将 {renamed_count} 个图标裁切为 {SQUARE_SIZE}x{SQUARE_SIZE} 圆角（半径 {CORNER_RADIUS}）并保存至 '{OUTPUT_DIR_NAME}' 目录。")

if __name__ == "__main__":
    crop_to_rounded_square()
